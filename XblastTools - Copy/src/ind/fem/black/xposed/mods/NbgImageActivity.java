...
public class NbgImageActivity extends Activity {
	
	 private File wallpaperImage;
     private File wallpaperTemporary;
     static final int NBG_IMAGE = 1025;
     static Context context;
	public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        context = getApplicationContext();
        wallpaperImage = new File(context.getFilesDir() + "/nbgImage");
        wallpaperTemporary = new File(context.getCacheDir() + "/nbgImage.tmp");
    ...
    private void setDefaultCallerImage() {
        ...
        try {
            ...
            startActivityForResult(intent,NBG_IMAGE);
            //((Activity) context).startActivityFromFragment(this, intent, LOCKSCREEN_BACKGROUND);
        } catch (Throwable e) {
        	XblastSettings.mPrefs.edit().putBoolean("nbgEnabled", false).commit();
            Toast.makeText(context, getString(
                    R.string.nbg_image_result_not_successful),
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == NBG_IMAGE) {
            if (resultCode == Activity.RESULT_OK) {
                if (wallpaperTemporary.exists()) {
                    wallpaperTemporary.renameTo(wallpaperImage);
                }
                wallpaperImage.setReadable(true, false);
                XblastSettings.mPrefs.edit().putBoolean("nbgEnabled", true).commit();
                Toast.makeText(context, getString(
                        R.string.nbg_image_result_successful),
                        Toast.LENGTH_SHORT).show();
            } else {
                if (wallpaperTemporary.exists()) {
