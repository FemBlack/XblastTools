...
import ind.fem.black.xposed.mods.Black.MethodState;
import ind.fem.black.xposed.quicksettings.AQuickSettingsTile;
import ind.fem.black.xposed.quicksettings.GravityBoxTile;
import ind.fem.black.xposed.quicksettings.QuickRecordTile;
import ind.fem.black.xposed.quicksettings.SleepTile;
import ind.fem.black.xposed.quicksettings.SyncTile;
import ind.fem.black.xposed.quicksettings.TorchTile;
import ind.fem.black.xposed.quicksettings.WifiApTile;
   ...
   /* private static ArrayList<AQuickSettingsTile> mTiles;
    ...
    private static BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(XblastSettings.ACTION_PREF_QUICKSETTINGS_CHANGED)) {
                if (intent.hasExtra(XblastSettings.EXTRA_QS_PREFS)) {
                    String[] qsPrefs = intent.getStringArrayExtra(XblastSettings.EXTRA_QS_PREFS);
                    mActiveTileKeys = new HashSet<String>(Arrays.asList(qsPrefs));
                    updateTileVisibility();
                }
                if (intent.hasExtra(XblastSettings.EXTRA_QS_COLS)) {
                    mNumColumns = intent.getIntExtra(XblastSettings.EXTRA_QS_COLS, 3);
                    if (mContainerView != null) {
                        XposedHelpers.callMethod(mContainerView, "updateResources");
                    }
                }
                if (intent.hasExtra(XblastSettings.EXTRA_QS_AUTOSWITCH)) {
                    mAutoSwitch = intent.getBooleanExtra(
                            XblastSettings.EXTRA_QS_AUTOSWITCH, false);
                }
                if (intent.hasExtra(XblastSettings.EXTRA_QUICK_PULLDOWN)) {
                    mQuickPulldown = intent.getIntExtra(
                    		XblastSettings.EXTRA_QUICK_PULLDOWN,
                            XblastSettings.QUICK_PULLDOWN_OFF);
                }
            }
        }
    };
    ...
    private static void updateTileVisibility() {
        ...
        for(String tileKey : mActiveTileKeys) {
            ...
            if (view == null) {
                ...
                view = mContainerView.findViewById(mXbContext.getResources().getIdentifier(
                        tileKey, "id", XblastSettings.PACKAGE_NAME));
            }
    ...
    public static void init(final XSharedPreferences prefs, final ClassLoader classLoader) {
        ...
        try {
            ...
            prefs.reload();
            mActiveTileKeys = prefs.getStringSet(XblastSettings.PREF_KEY_QUICK_SETTINGS, null);
            log("got tile prefs: mActiveTileKeys = " + (mActiveTileKeys == null ? "null" : mActiveTileKeys.toString()));

            try {
                mNumColumns = prefs.getInt(XblastSettings.PREF_KEY_QUICK_SETTINGS_TILES_PER_ROW, 3);
            } catch (NumberFormatException e) {
                log("Invalid preference for tiles per row: " + e.getMessage());
            }

            mAutoSwitch = prefs.getBoolean(XblastSettings.PREF_KEY_QUICK_SETTINGS_AUTOSWITCH, false);

            try {
                mQuickPulldown = Integer.valueOf(prefs.getString(
                        XblastSettings.PREF_KEY_QUICK_PULLDOWN, "0"));
            } catch (NumberFormatException e) {
    ...
    private static XC_MethodHook quickSettingsConstructHook = new XC_MethodHook() {
        ...
        protected void afterHookedMethod(final MethodHookParam param) throws Throwable {
            ...
            mXbContext = mContext.createPackageContext(XblastSettings.PACKAGE_NAME, Context.CONTEXT_IGNORE_SECURITY);
            mContainerView = (ViewGroup) XposedHelpers.getObjectField(param.thisObject, "mContainerView");

            IntentFilter intentFilter = new IntentFilter(XblastSettings.ACTION_PREF_QUICKSETTINGS_CHANGED);
    ...
    private static XC_MethodHook quickSettingsAddSystemTilesHook = new XC_MethodHook() {
        ...
        protected void afterHookedMethod(final MethodHookParam param) throws Throwable {
            ...
            NetworkModeTile nmTile = new NetworkModeTile(mContext, mXbContext, mStatusBar, mPanelBar);
            nmTile.setupQuickSettingsTile(mContainerView, inflater);
            mTiles.add(nmTile);

            SyncTile syncTile = new SyncTile(mContext, mXbContext, mStatusBar, mPanelBar);
            syncTile.setupQuickSettingsTile(mContainerView, inflater);
            mTiles.add(syncTile);

            WifiApTile wifiApTile = new WifiApTile(mContext, mXbContext, mStatusBar, mPanelBar);
            wifiApTile.setupQuickSettingsTile(mContainerView, inflater);
            mTiles.add(wifiApTile);

            TorchTile torchTile = new TorchTile(mContext, mXbContext, mStatusBar, mPanelBar);
            torchTile.setupQuickSettingsTile(mContainerView, inflater);
            mTiles.add(torchTile);

            SleepTile sleepTile = new SleepTile(mContext, mXbContext, mStatusBar, mPanelBar);
            sleepTile.setupQuickSettingsTile(mContainerView, inflater);
            mTiles.add(sleepTile);

            QuickRecordTile qrTile = new QuickRecordTile(mContext, mXbContext, mStatusBar, mPanelBar);
            qrTile.setupQuickSettingsTile(mContainerView, inflater);
            mTiles.add(qrTile);

            GravityBoxTile gbTile = new GravityBoxTile(mContext, mXbContext, mStatusBar, mPanelBar);
                ...
                switch (event.getActionMasked()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        if (mAutoSwitch &&
                            ...
                            shouldFlip = true;
                        } else if (mQuickPulldown == XblastSettings.QUICK_PULLDOWN_RIGHT
                                    && (event.getX(0) > (width *
                                    (1.0f - STATUS_BAR_SETTINGS_FLIP_PERCENTAGE_RIGHT)))) {
                            shouldFlip = true;
                        } else if (mQuickPulldown == XblastSettings.QUICK_PULLDOWN_LEFT
                                    && (event.getX(0) < (width *
